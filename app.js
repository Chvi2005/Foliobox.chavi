...
/* ==========================================================================
   ADMIN PANEL & GALLERY EVENT PHOTO UPLOAD SYSTEM
   ========================================================================== */
const STORAGE_KEY_GALLERY = 'portfolio_uploaded_gallery_items';
...
function getUploadedPhotos() {
  try {
    ...
    console.error('Error reading gallery items from storage', e);
    return [];
  }
}

function saveUploadedPhoto(photoObj) {
  try {
    const current = getUploadedPhotos();
    current.unshift(photoObj); // Add to beginning
    localStorage.setItem(STORAGE_KEY_GALLERY, JSON.stringify(current));
    renderUploadedGallery();
  } catch (e) {
    console.error('Error saving photo to storage', e);
    alert('Storage limit reached or failed to save photo. Try a smaller image.');
  }
}

function deleteUploadedPhoto(id) {
  try {
    let current = getUploadedPhotos();
...
function renderUploadedGallery() {
  ...
  // 1. Update Admin Modal Uploaded Count & List
  const countEl = document.getElementById('admin-uploaded-count');
  if (countEl) countEl.textContent = uploadedPhotos.length;

  const adminList = document.getElementById('admin-uploaded-items-list');
  if (adminList) {
    if (uploadedPhotos.length === 0) {
      adminList.innerHTML = `<p class="v4-role" style="grid-column: 1/-1; text-align: center; padding: 20px;">No event photos uploaded yet.</p>`;
    } else {
      adminList.innerHTML = uploadedPhotos.map(photo => `
        <div class="masonry-item" style="position: relative;" data-lightbox-src="${photo.src}" data-lightbox-title="${escapeHtml(photo.title)}" data-lightbox-desc="${escapeHtml(photo.desc)}">
          <button class="btn-delete-uploaded" data-delete-id="${photo.id}" title="Delete Photo">&times;</button>
          <img src="${photo.src}" alt="${escapeHtml(photo.title)}">
          <div class="masonry-overlay">
            <span class="masonry-title">${escapeHtml(photo.title)}</span>
            <span class="masonry-sub">${escapeHtml(photo.category)}</span>
          </div>
        </div>
      `).join('');
    }
  ...
  // 2. Update Home Page Gallery Summary Grid (#gallery-summary-grid)
  const homeGalleryGrid = document.getElementById('gallery-summary-grid');
  if (homeGalleryGrid) {
    // Preserve static initial items if present, or rebuild dynamically
    const staticHomeCards = [
      {
        src: './images/gallery_wesak_dansal.jpg',
        title: 'Wesak Dansal (Team Valkyries)',
        category: 'Event',
        desc: 'Community ice cream dansal organized by Team Valkyries.'
      },
      {
        src: './images/gallery_emp_module.jpg',
        title: 'EMP Module (Team Valkyries)',
        category: 'Project',
        desc: 'Collaborative technical workshop developing the EMP module.'
      },
      {
        src: './images/gallery_field_trip_ambewella.jpg',
        title: 'Field trip Ambewella (Team Valkyries)',
        category: 'Event',
        desc: 'Team Valkyries educational field trip to Ambewella.'
      }
    ];

    const allHomeItems = [
      ...uploadedPhotos.map(p => ({ ...p, isUploaded: true })),
      ...staticHomeCards
    ];

    homeGalleryGrid.innerHTML = allHomeItems.map(item => `
      <div class="gallery-card reveal-on-scroll is-visible" data-lightbox-src="${item.src}" data-lightbox-title="${escapeHtml(item.title)}" data-lightbox-desc="${escapeHtml(item.desc)}">
        ${item.isUploaded ? `<button class="btn-delete-uploaded" data-delete-id="${item.id}" title="Delete Photo">&times;</button>` : ''}
        <div class="gallery-card-img">
          <img src="${item.src}" alt="${escapeHtml(item.title)}">
          <div class="gallery-badge">${escapeHtml(item.category || 'Event')}</div>
        </div>
  ...
  // 3. Update Full Gallery Modal (#full-gallery-grid)
  const fullGalleryGrid = document.getElementById('full-gallery-grid');
  if (fullGalleryGrid) {
    const staticFullCards = [
      {
        src: './images/gallery_wesak_dansal.jpg',
        title: 'Wesak Dansal (Team Valkyries)',
        category: 'Event',
    ...
    fullGalleryGrid.innerHTML = allFullItems.map(item => `
      <div class="masonry-item" style="position: relative;" data-lightbox-src="${item.src}" data-lightbox-title="${escapeHtml(item.title)}" data-lightbox-desc="${escapeHtml(item.desc)}">
        ${item.isUploaded ? `<button class="btn-delete-uploaded" data-delete-id="${item.id}" title="Delete Photo">&times;</button>` : ''}
        <img src="${item.src}" alt="${escapeHtml(item.title)}">
        <div class="masonry-overlay">
          <span class="masonry-title">${escapeHtml(item.title)}</span>
          <span class="masonry-sub">${escapeHtml(item.category || 'Event')}</span>
        </div>
      </div>
    `).join('');
  }
...
function initAdminGalleryManager() {
  ...
  const removePreviewBtn = document.getElementById('admin-remove-preview-btn');
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      const imgSrc = currentImageData || (urlInput ? urlInput.value.trim() : '');
      ...
      saveUploadedPhoto(photoObj);

      // Reset form
      form.reset();
      clearPreview();

      // Show success modal & optionally close admin modal
      closeModal('admin-modal');
      openModal('success-modal');
    });
  }

  // Event Delegation for Delete Photo Buttons
  document.addEventListener('click', (e) => {
    const deleteBtn = e.target.closest('.btn-delete-uploaded');
    if (deleteBtn) {
      e.preventDefault();
      e.stopPropagation();
      const id = deleteBtn.getAttribute('data-delete-id');
      if (id && confirm('Are you sure you want to delete this photo from the gallery?')) {
        deleteUploadedPhoto(id);
      }
    }
  });

  // Initial render of uploaded gallery items
  renderUploadedGallery();
}
